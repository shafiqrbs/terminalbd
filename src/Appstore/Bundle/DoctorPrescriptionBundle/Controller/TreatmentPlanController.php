...
use Knp\Snappy\Pdf;
use Appstore\Bundle\DoctorPrescriptionBundle\Entity\DmsInvoice;
use Appstore\Bundle\DoctorPrescriptionBundle\Entity\DmsInvoiceMedicine;
use Appstore\Bundle\DoctorPrescriptionBundle\Entity\DmsInvoiceParticular;
use Appstore\Bundle\DoctorPrescriptionBundle\Entity\DmsParticular;
use Appstore\Bundle\DoctorPrescriptionBundle\Entity\DmsTreatmentPlan;
...
class TreatmentPlanController extends Controller
{
    ...
    public function indexAction()
    {
        ...
        $treatmentSchedule  = $em->getRepository('DoctorPrescriptionBundle:DpsTreatmentPlan')->findTodaySchedule($config,$data);
        ...
        return $this->render('DoctorPrescriptionBundle:Invoice:treatmentSchedule.html.twig', array(
            'entities' => $pagination,
    ...
    public function sendSmsPlanAction($patient, DmsTreatmentPlan $plan)
    {
        ...
        $dispatcher->dispatch('setting_tool.post.dps_treatment_plan_sms', new \Setting\Bundle\ToolBundle\Event\DmsTreatmentPlanSmsEvent($plan));
        $plan->setSendSms(1);
        $em->flush();
        exit;
    }
    ...
    public function appointmentDateScheduleAction(Request $request)
    {
        ...
        $assignDoctors = $this->getDoctrine()->getRepository('DoctorPrescriptionBundle:DpsParticular')->getFindWithParticular($config,array('doctor'));
        $treatments = $this->getDoctrine()->getRepository('DoctorPrescriptionBundle:DpsParticular')->getFindDentalServiceParticular($config,array('treatment'));
        $treatmentSchedule  = $em->getRepository('DoctorPrescriptionBundle:DpsTreatmentPlan')->findTodaySchedule($config,$data = array());
        $html =  $this->renderView('DoctorPrescriptionBundle:Invoice:schedule-plan.html.twig', array(
    ...
    public function dateWiseScheduleAction(Request $request)
    {
        ...
        $appointments = $this->getDoctrine()->getRepository('DoctorPrescriptionBundle:DpsTreatmentPlan')->appointmentDate($dmsConfig,$data);
    ...
    public function appointmentFreeTimeScheduleAction()
    {
        ...
        $data = $_REQUEST;
        $dmsConfig = $user->getGlobalOption()->getDmsConfig();
        $appointments = $this->getDoctrine()->getRepository('DoctorPrescriptionBundle:DpsTreatmentPlan')->findFreeAppointmentTime($dmsConfig,$data);
