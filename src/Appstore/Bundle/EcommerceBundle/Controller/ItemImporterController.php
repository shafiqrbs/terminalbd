...
use Appstore\Bundle\EcommerceBundle\Form\ItemImporterType;
...
class ItemImporterController extends Controller
{
    ...
    public function indexAction()
    {
        ...
        $entities = $em->getRepository('EcommerceBundle:ItemImport')->findBy(array('ecommerceConfig'=>$config),array('updated'=>'DESC'));
        return $this->render('EcommerceBundle:ItemImporter:index.html.twig', array(
            'entities' => $entities,
        ));
    }
    /**
    ...
    public function createAction(Request $request)
    {
        ...
        if ($form->isValid()) {
            ...
            return $this->redirect($this->generateUrl('ecommerce_itemimporter'));
        }
    ...
    private function createCreateForm(ItemImport $entity)
    {
        ...
        $form = $this->createForm(new ItemImporterType($globalOption), $entity, array(
            'action' => $this->generateUrl('ecommerce_itemimporter_create'),
            'method' => 'POST',
            'attr' => array(
    ...
    public function excelDataImportAction(ItemImport $ItemImporter)
    {
        ...
        $em = $this->getDoctrine()->getManager();
        $importer = $this->get('appstore_inventory_item_import');
        $importer->setItemImport($ItemImporter);
        $reader = $this->get('appstore_inventory.importer.item_excel_data_reader');
    ...
    */
    private function createEditForm(ItemImport $entity)
    {
        $globalOption = $this->getUser()->getGlobalOption();
        $form = $this->createForm(new ItemImporterType($globalOption), $entity, array(
            'action' => $this->generateUrl('ecommerce_itemimporter_update', array('id' => $entity->getId())),
            'method' => 'PUT',
        ));
        return $form;
    }
    ...
    public function updateAction(Request $request, $id)
    {
        ...
        if ($editForm->isValid()) {
            $em->flush();
            return $this->redirect($this->generateUrl('ecommerce_itemimporter_edit', array('id' => $id)));
        }

        return $this->render('EcommerceBundle:ItemImporter:edit.html.twig', array(
            'entity'      => $entity,
            'edit_form'   => $editForm->createView(),
