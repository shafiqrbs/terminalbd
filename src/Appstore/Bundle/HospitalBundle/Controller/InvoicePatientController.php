...
class InvoicePatientController extends Controller
{
    ...
    public function newAction()
    {
        ...
        $em->persist($entity);
        $em->flush();
        return $this->redirect($this->generateUrl('hms_invoice_patient_edit', array('id' => $entity->getId())));
    ...
    public function editAction($id)
    {
        ...
        if ($entity->getProcess() != "In-progress") {
            return $this->redirect($this->generateUrl('hms_invoice_patient_show', array('id' => $entity->getId())));
        }
        ...
        $data = $request->request->all()['appstore_bundle_hospitalbundle_invoice'];
         if ($editForm->isValid()) {
            ...
            if(!empty($this->getUser()->getGlobalOption()->getNotificationConfig()) and  !empty($this->getUser()->getGlobalOption()->getSmsSenderTotal())) {
               ...
               $dispatcher->dispatch('setting_tool.post.hms_invoice_patient_sms', new \Setting\Bundle\ToolBundle\Event\HmsInvoiceSmsEvent($entity));
            }
            return $this->redirect($this->generateUrl('hms_invoice_patient_show', array('id' => $entity->getId())));
        }
    ...
    private function createEditForm(Invoice $entity)
    {
        ...
        $form = $this->createForm(new InvoiceType($globalOption,$category ,$location), $entity, array(
            'action' => $this->generateUrl('hms_invoice_patient_update', array('id' => $entity->getId())),
            'method' => 'PUT',
            'attr' => array(
    ...
    public function deleteEmptyInvoiceAction()
    {
        ...
        return $this->redirect($this->generateUrl('hms_invoice_patient'));
    }
