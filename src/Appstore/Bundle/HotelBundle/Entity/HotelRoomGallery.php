 ...
 * ProductGallery
 *
 * @ORM\Table("hotel_room_gallery")
 * @ORM\Entity(repositoryClass="Appstore\Bundle\HotelBundle\Repository\HotelRoomGalleryRepository")
 */
class HotelRoomGallery
{
    /**
     * @var integer
     *
    ...
    public function getWebPath($globalOption,$item)
    {
        return null === $this->path
            ? null
            : $this->getUploadDir().'/'.$globalOption.'/hotel/item/'.$item.'/gallery/'.$this->path;
    }

    protected function getUploadRootDir($globalOption,$item)
    {
      //return 'uploads/files/'.$user.'/inventory/item/'.$item.'/gallery/';
      return WEB_PATH.'/'.$this->getUploadDir().'/'.$globalOption.'/hotel/item/'.$item.'/gallery/';
    }
    ...
    public function upload($globalOption,$item)
    {
        ...
        die('{"jsonrpc" : "2.0", "result" : null, "id" : $globalOption }');
