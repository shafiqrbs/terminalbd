 ...
 *
 * @ORM\Table()
 * @ORM\Entity(repositoryClass="Appstore\Bundle\InventoryBundle\Repository\ItemGalleryRepository")
 */
class ItemGallery
{
    /**
    ...
    public function getWebPath($globalOption,$item)
    {
        return null === $this->path
            ? null
            : $this->getUploadDir().'/'.$globalOption.'/inventory/item/'.$item.'/gallery/'.$this->path;
    }

    protected function getUploadRootDir($globalOption,$item)
    {
      //return 'uploads/files/'.$user.'/inventory/item/'.$item.'/gallery/';
      return WEB_PATH.'/'.$this->getUploadDir().'/'.$globalOption.'/inventory/item/'.$item.'/gallery/';
    }
