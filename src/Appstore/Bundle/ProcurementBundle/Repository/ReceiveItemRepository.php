 ...
 */
class ReceiveItemRepository extends EntityRepository
{
    ...
    public function insertReceiveItem($entity,PurchaseItem $purchaseItem){

        $em = $this->_em;
        $exist = $this->findOneBy(array('receive'=>$entity,'purchaseItem'=>$purchaseItem));
        if(empty($exist)){
            $item = new ReceiveItem();
            $item->setReceive($entity);
            $item->setPurchaseItem($purchaseItem);
            $item->setItem($purchaseItem->getItem());
            $item->setQuantity($purchaseItem->getRemainingQuantity());
            $item->setPoQuantity($purchaseItem->getRemainingQuantity());
            $item->setPrice($purchaseItem->getPurchasePrice());
            $item->setSubTotal($purchaseItem->getRemainingQuantity() * $purchaseItem->getPurchasePrice());
            $em->persist($item);
        }
        $em->flush();
    ...
    public function updatePurchaseTotalPrice(StockIssue $entity)
    {
        $em = $this->_em;
        $total = $this->createQueryBuilder('si')
            ->join('si.stockIssue','e')
