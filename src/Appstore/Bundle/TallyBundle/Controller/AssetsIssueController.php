...
use Appstore\Bundle\TallyBundle\Form\AssetsIssueType;
...
class AssetsIssueController extends Controller
{
    ...
    public function indexAction()
    {
        ...
        $data = $_REQUEST;
        $entities = $em->getRepository('TallyBundle:Sales')->salesLists( $this->getUser() , $mode = 'assets', $data);
        $pagination = $this->paginate($entities);
        $transactionMethods = $em->getRepository('SettingToolBundle:TransactionMethod')->findBy(array('status' => 1), array('name' => 'ASC'));
        return $this->render('TallyBundle:AssetsIssue:index.html.twig', array(
            'entities' => $pagination,
            'config' => $config,
    ...
    public function newAction()
    {
        ...
        $em->persist($entity);
        $em->flush();
        return $this->redirect($this->generateUrl('tally_assetsissue_edit', array('code' => $entity->getInvoice())));
    ...
    public function editAction($code)
    {
        ...
        if(!in_array($entity->getProcess(),array('In-progress','Created'))) {
            return $this->redirect($this->generateUrl('tally_assetsissue_show', array('id' => $entity->getId())));
        }
        ...
        return $this->render('TallyBundle:AssetsIssue:sales.html.twig', array(
            'entity' => $entity,
    ...
    {
        $globalOption = $this->getUser()->getGlobalOption();
        $form = $this->createForm(new AssetsIssueType($globalOption), $entity, array(
            'action' => $this->generateUrl('tally_assetsissue_update', array('id' => $entity->getId())),
            'method' => 'PUT',
            'attr' => array(
    ...
    public function showAction(Sales $entity)
    {
        ...
        if ($inventory->getId() == $entity->getConfig()->getId()) {
            return $this->render('TallyBundle:AssetsIssue:show.html.twig', array(
                'entity' => $entity,
                'config' => $inventory,
            ));
        } else {
            return $this->redirect($this->generateUrl('tally_assetsissue'));
        }
    ...
    public function updateAction(Request $request, Sales $entity)
    {
        ...
        if($editForm->isValid()) {
            ...
            $em->flush();
            return $this->redirect($this->generateUrl('tally_assetsissue_show'));
        }
    ...
    public function approveAction(Sales $entity)
    {
        if (!empty($entity) and $entity->getProcess() == "Done") {
            ...
            // $em->getRepository('AccountingBundle:Transaction')->tallySalesTransaction($entity);
