...
class IssueController extends Controller
{
    ...
    private function createEditForm(Sales $entity)
    {
        ...
        $form = $this->createForm(new IssueType($globalOption,$location), $entity, array(
    ...
    public function salesItemAction()
    {
        ...
        $pagination = $this->paginate($entities);
        return $this->render('TallyBundle:Invoice:salesItem.html.twig', array(
            'entities' => $pagination,
            'searchForm' => $data,
        ));
    }
    ...
    public function salesDiscountUpdateAction(Request $request)
    {
        ...
        if($total > 0 ){
            ...
            if ($sales->getConfig()->getVatEnable() == 1 && $sales->getConfig()->getVatPercentage() > 0) {
    ...
    {
        $inventory = $this->getUser()->getGlobalOption()->getTallyConfig();
        $entities = $this->getDoctrine()->getRepository('TallyBundle:Sales')->findBy(array('config' => $inventory, 'paymentStatus' => 'Pending', 'process' => 'Created'));
        $em = $this->getDoctrine()->getManager();
    ...
    public function onlinePosPrintAction(Request $request)
    {
        ...
        $inventory = $this->getUser()->getGlobalOption()->getTallyConfig();
        $entity = $this->getDoctrine()->getRepository('TallyBundle:Sales')->findOneBy(array('config' => $inventory, 'id' => $salesId));
    ...
    public function updateOnlineSalesByPosPrint(Sales $entity,$data){
        ...
        /*
        if (in_array('OnlineSales', $entity->getConfig()->getDeliveryProcess())) {
